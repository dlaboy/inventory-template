...
$conn = pg_connect("host=cbdhrtd93854d5.cluster-czrs8kj4isg7.us-east-1.rds.amazonaws.com port=5432 dbname=d5fpodsd053b94 user=ubl8ijvajs5g5i password=pce5349717e3540b10a0feae772a14952d15888812769265104b0a05cdf18892c");
...
if (isset($_POST['siu'])) {
    if (!empty($_POST['i'])) {
        $item = pg_escape_string($conn, $_POST['i']);

        $sqlSearched = "SELECT * FROM comestibles WHERE item = '$item'";
        $sql1 = 'SELECT * FROM no_comestibles ORDER BY quantity ASC';


        $search = pg_query($conn, $sqlSearched);
        $result1 = pg_query($conn, $sql1);


        $comestibles = pg_fetch_all($search);
        $no_comestibles = pg_fetch_all($result1);
        ...
        $sql11 = 'SELECT * FROM no_comestibles ORDER BY quantity ASC';

        // make query & get result
        $result = pg_query($conn, $sql);
        $result1 = pg_query($conn, $sql11);



        // fetch the resulting rows as an array 
        $comestibles = pg_fetch_all($result);
        $no_comestibles = pg_fetch_all($result1);
    ...
    if (!empty($_POST['i2'])) {
        $item = pg_escape_string($conn, $_POST['i2']);

        $sqlSearched = "SELECT * FROM no_comestibles WHERE item = '$item'";
        ...
        $comestibles = pg_fetch_all($result1);
        $no_comestibles = pg_fetch_all($search);
...
if (isset($_POST['increment'])) {
    ...
    $id_to_increment  = pg_escape_string($conn, $_POST['id_to_increment']);


    $sqli = "UPDATE comestibles SET quantity = quantity +'1' WHERE id = $id_to_increment";


    $increment = pg_query($conn, $sqli);
...
if (isset($_POST['increment1'])) {
    $id_to_increment1  = pg_escape_string($conn, $_POST['id_to_increment1']);


    $sql = "UPDATE no_comestibles SET quantity = quantity +'1' WHERE id = $id_to_increment1";
...
if (isset($_POST['delete'])) {
    $id_to_delete  = pg_escape_string($conn, $_POST['id_to_delete']);
...
if (isset($_POST['delete1'])) {
    $id_to_delete1  = pg_escape_string($conn, $_POST['id_to_delete1']);

    $sql1 = "DELETE FROM no_comestibles WHERE id = $id_to_delete1";
...
if (isset($_POST['submit1'])) { // check item quantity
    ...
    if (array_filter($errors1)) {
        ...
        // echo 'Item added';
        // OJO
        $name1 = pg_escape_string($conn, $_POST['name1']);
        $quantity1 = pg_escape_string($conn, $_POST['quantity1']);

        // create sql

        $sql1 = "INSERT INTO no_comestibles(item, quantity) VALUES('$name1', '$quantity1')";
...
<head>
    ...
    <title>Oasis POS: Inventory</title>
</head>
    ...
    <div class="content container mt-4">
        ...
        <div class="rowa row shadow rounded-4 mt-4 p-3">
            ...
            <table id="nt" class=" table table-hover">
                ...
                <tbody>
                    <?php foreach ($no_comestibles as $noncomestible) { ?>
